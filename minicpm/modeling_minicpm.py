...
class MiniCPMForCausalLM(MiniCPMPreTrainedModel):
    ...
    def stream_generate(
        ...
        if self.is_length_limit is True:
            print(None, None)
        ...
        if self.history[-1]["content"] == "":
            self.history[-1]["timestamp"] = time.time()
        self.history[-1]["content"] += response[cut_len:]
        return response[cut_len:], self.history

    @torch.inference_mode()
    def chat(self, tokenizer, query: str, role: str = "user",
        ...
        self.history.append({"role": role, "content": query, "timestamp": time.time()})
        # history_str = tokenizer.apply_chat_template(self.history, tokenize=False, add_generation_prompt=False)
